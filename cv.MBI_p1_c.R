...
cv.MBI_p1 <- function (x, y, x.ori=NULL, start_source, standardize=TRUE, lambda, lambda2, r_va=0.2, nfolds=10, alpha=0.05, debiased=1:p, parallel=FALSE, parallel_LP=FALSE, b_count_max=10, use_true=FALSE, bound0=0.1) {
  ...
  for (t in 1:nfolds){
    ...
    if (nlam>1) {
      model_fold = MBI_ini(x_train = x3_train, y_train = y_train, x5_train = x5_train, 
                           x_test = x3_test, y_test = y_test, parallel_LP=parallel_LP,
  ...
  model = MBI_ini(x_train = x4, y_train = y4, x5_train = x6, parallel_LP=parallel_LP,
                  start_source = start_source, lambda = lambda,
  ...
  #   beta_final[j]=t(v.min[,j])%*%(model$b1-model$A1[,-j]%*%beta.min[-j])/(t(v.min[,j])%*%model$A1[,j])
  ...
  returnlist=list("beta.min"=beta_out.min, "beta_final"=beta_final_out,
                  ...
                  "PMSE"=PMSE, "Pvar"=Pvar, "v.min"=v.min,
                  "lambda"=lambda, "lambda2"=lambda2, "s_square"=s_square,
                  "impest"=impest, "debiased"=debiased, "sigma_squared"=sigma_squared,
...
weight.v <- function (x_train, y_train, x5_train, x_test=NULL, y_test=NULL, x5_test=NULL, 
  ...
  for (i in 1:n_pat) {
    ...
    for (j in ug[[i]]) {
      ...
      k=which((pat==i) & (pat_ug==j))
      A1=rbind(A1, t(x_train[index_temp,ak,k])%*%x_train[index_temp,,k]/length(index_temp))
      count_temp=sum(!is.na(x5_train[pat_start[j],]))
  ...
  #start.time = proc.time()
  if (parallel) {
    results_j=foreach (j = debiased, .packages="kernlab") %dopar% {
      v_j=matrix(0,nlam2,N_g)
      Tvar_j=rep(0,nlam2)
      Pvar_j=rep(0,nlam2)
    ...
    results_j=foreach (j = debiased, .packages="kernlab") %do% {
      v_j=matrix(0,nlam2,N_g)
      Tvar_j=rep(0,nlam2)
      Pvar_j=rep(0,nlam2)
      v_status_j=rep(TRUE,nlam2)
